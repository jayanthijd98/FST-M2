-- Load Data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/jayanthi/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/jayanthi/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/jayanthi/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/jayanthi/outputs;

-- Store Result in HDFS
STORE nameOrdered INTO 'hdfs:///user/jayanthi/outputs' USING PigStorage('\t');